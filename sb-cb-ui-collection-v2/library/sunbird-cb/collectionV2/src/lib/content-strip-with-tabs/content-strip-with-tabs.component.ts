  ...
  async fetchFromSearchV6(strip: NsContentStripWithTabs.IContentStripUnit, calculateParentStatus = true) {
    if (strip.request && strip.request.searchV6 && Object.keys(strip.request.searchV6).length) {
      ...
      if (strip.request &&
        ...
        strip.request.searchV6.filters) {
        originalFilters = strip.request.searchV6.filters
        strip.request.searchV6.filters = this.checkForDateFilters(strip.request.searchV6.filters)
        strip.request.searchV6.filters = this.getFiltersFromArray(
          strip.request.searchV6.filters,
        )
      }
      if (strip.tabs && strip.tabs.length) {
    ...
    return new Promise<any>((resolve, reject) => {
      if (request && request.searchV6) {
        this.contentSvc.searchV6(request.searchV6).subscribe(results => {
          const showViewMore = Boolean(
            results.result && results.result.length > 5 && strip.stripConfig && strip.stripConfig.postCardForSearch,
          )
          const viewMoreUrl = showViewMore
            ? {
              path: strip.viewMoreUrl && strip.viewMoreUrl.path || '',
  ...
  async trendingSearchRequest(strip: NsContentStripWithTabs.IContentStripUnit,
    ...
    return new Promise<any>((resolve, reject) => {
      if (request && request.trendingSearch) {
        ...
        // COMMENTED OUT: trendingContentSearch method doesn't exist on WidgetContentService
        // this.contentSvc.trendingContentSearch(request.trendingSearch).subscribe(results => { ... })
        // Fallback error handling
        reject('trendingContentSearch not available')
      }
    })
  }
