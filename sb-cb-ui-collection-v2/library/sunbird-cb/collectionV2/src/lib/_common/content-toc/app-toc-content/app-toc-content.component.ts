  ...
  ngOnInit() {
    ...
    this.typesOfContent = [
      ...
      },
      {
        ...
        id: 'session',
        disabled: false,
        message: `Note: The session handouts will be visible to
        learners once they log in to the iGOT Karmayogi portal and enroll in the batch.`,
      },
    ]
    ...
    if (this.tocStructure) {
      ...
      if (!this.otherResourse) {
        setTimeout(() => {
          ...
          this.selectedTabMessage = this.typesOfContent.find((ele: any) => ele.id === this.selectedTabType).message
