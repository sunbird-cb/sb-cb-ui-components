...
import { NsAppToc, AppTocService } from '../../../_services/app-toc.service'
...
export class AppTocContentCardV2Component implements OnInit {
  ...
  getDuration(identifier: any) {
    let duration = 0
    if (this.hierarchyMapData && this.hierarchyMapData.identifier === identifier) {
      if (this.hierarchyMapData.duration) {
        duration = duration + Number(this.hierarchyMapData.duration)
      }
      if (this.hierarchyMapData.expectedDuration) {
        duration = duration + Number(this.hierarchyMapData.expectedDuration)
      }
    }
    else if (this.hierarchyMapData && this.hierarchyMapData.children && this.hierarchyMapData.children.length) {
      this.hierarchyMapData.children.forEach((item: any) => {
        if (item.children && item.children.length) {
          item.children.forEach((childItem: any) => {
            if (childItem && childItem.children && childItem.children.length) {
              childItem.children.forEach((childItem2: any) => {
                if (childItem.parent === identifier) {
                  if (childItem2.duration) {
                    duration = duration + Number(childItem2.duration)
                  }
                  if (childItem2.expectedDuration) {
                    duration = duration + Number(childItem2.expectedDuration)
                  }
                }
            ...
            } else if (childItem.parent === identifier) {
              if (childItem.duration) {
                duration = duration + Number(childItem.duration)
              }
              if (childItem.expectedDuration) {
                duration = duration + Number(childItem.expectedDuration)
              }
          ...
          if (item.duration) {
            duration = duration + Number(item.duration)
          }
          if (item.expectedDuration) {
            duration = duration + Number(item.expectedDuration)
          }
          // if (item.children && item.children.length) {
          //   item.children.forEach((childItem: any) => {
          //     if (childItem.duration) {
