...
export class AppTocSessionsNewComponent implements OnInit {
  ...
  sessionListMap: any = {}
  sessionType: any = []
  batchOfflineOnlineSessionData: any = []
  constructor() { }
      ...
      if (this.content && this.content.children) {
        for (let i = 0; i < this.content.children.length; i++) {
          if (this.content.children[i] && (
            this.content.children[i]['primaryCategory'] === 'Offline Session'
          )
          ) {
            this.batchOfflineOnlineSessionData.push(this.content.children[i])
          }
          if (this.content.children[i] && this.content.children[i].children && this.content.children[i].children.length) {
            for (let j = 0; j < this.content.children[i].children.length; j++) {
              if (this.content.children[i].children[j] && (
                this.content.children[i].children[j]['primaryCategory'] === 'Offline Session'
              )
              ) {
                this.batchOfflineOnlineSessionData.push(this.content.children[i].children[j])
              }
            }
          }
        }
