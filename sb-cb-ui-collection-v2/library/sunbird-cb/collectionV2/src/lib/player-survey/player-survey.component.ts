...
export class PlayerSurveyComponent extends WidgetBaseComponent
  ...
  ngOnInit() {
    ...
    this.wfClientVersion = _.get(this.activatedRoute, 'snapshot.data.content.data.wfClientVersion', '0')
    this.getFormDetails()
    this.apiData = {
      // tslint:disable-next-line:prefer-template
    ...
    this.activatedRoute.params.subscribe(params => {
      ...
      if (identifier && batchId && this.configSvc.userProfile) {
        ...
        if (this.activatedRoute.snapshot.queryParams.collectionId &&
          ...
          this.identifierId) {
          // TODO: ViewerUtilService needs to be properly imported
          ...
          const resData = { batchId: this.activatedRoute.snapshot.queryParams.batchId, courseId: this.activatedRoute.snapshot.queryParams.collectionId }
          const req: any = {
            request: {
              userId,
          ...
          this.widgetServ.fetchContentHistoryV2(req).subscribe(
            (data: any) => {
              ...
              // TODO: setProgramChildResumeData method needs to be implemented in WidgetContentService
