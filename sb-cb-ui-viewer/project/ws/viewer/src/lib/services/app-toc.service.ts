...
export class AppTocService {
  ...
  initData(data: Data, needResumeData: boolean = false): NsAppToc.IWsTocResponse {
    let content: NsContent.IContent | null | any = null
  ...
  getTocStructure(
    ...
    if (
      ...
      if (content.primaryCategory === NsContent.EPrimaryCategory.COURSE) {
        tocStructure.course = (tocStructure.course || 0) + 1
      } else if (content.primaryCategory === NsContent.EPrimaryCategory.MODULE) {
        tocStructure.learningModule = (tocStructure.learningModule || 0) + 1
      }
      ...
      switch (content.mimeType) {
        ...
        case NsContent.EMimeTypes.MP3:
          tocStructure.podcast = (tocStructure.podcast || 0) + 1
          break
        case NsContent.EMimeTypes.MP4:
        case NsContent.EMimeTypes.M3U8:
        case NsContent.EMimeTypes.YOUTUBE:
          tocStructure.video = (tocStructure.video || 0) + 1
          break
        // case NsContent.EMimeTypes.INTERACTION:
        //   tocStructure.interactiveVideo += 1
        //   break
        case NsContent.EMimeTypes.PDF:
          tocStructure.pdf = (tocStructure.pdf || 0) + 1
          break
        // case NsContent.EMimeTypes.HTML:
        case NsContent.EMimeTypes.TEXT_WEB:
          tocStructure.webPage = (tocStructure.webPage || 0) + 1
          break
        case NsContent.EMimeTypes.SURVEY:
          tocStructure.survey = (tocStructure.survey || 0) + 1
          break
        case NsContent.EMimeTypes.QUIZ:
        case NsContent.EMimeTypes.APPLICATION_JSON:
          // if (content.resourceType === 'Assessment') {
          tocStructure.assessment = (tocStructure.assessment || 0) + 1
          // } else {
          //   tocStructure.quiz += 1
          // }
          break
        case NsContent.EMimeTypes.OFFLINE_SESSION:
          // if (content.resourceType === 'Assessment') {
          tocStructure.offlineSession = (tocStructure.offlineSession || 0) + 1
          // } else {
          //   tocStructure.quiz += 1
          // }
          break
        case NsContent.EMimeTypes.PRACTICE_RESOURCE:
          if (content.primaryCategory === this.primaryCategory.PRACTICE_RESOURCE) {
            tocStructure.practiceTest = (tocStructure.practiceTest || 0) + 1
          } else if (content.primaryCategory === this.primaryCategory.FINAL_ASSESSMENT) {
            tocStructure.finalTest = (tocStructure.finalTest || 0) + 1
          }
          break
        ...
        case NsContent.EMimeTypes.ZIP:
          tocStructure.interactivecontent = (tocStructure.interactivecontent || 0) + 1
          break
        // case NsContent.EMimeTypes.YOUTUBE:
        //   tocStructure.youtube += 1
        //   break
        default:
          tocStructure.other = (tocStructure.other || 0) + 1
          break
      }
      return tocStructure
    }
  ...
  async mapCompletionPercentageProgram(content: NsContent.IContent | null | any, enrolmentList: any) {
    this.contentLoader.next(true)
    let totalCount = 0
    let leafnodeCount = 0
    ...
    if (content && content.children) {
      ...
      if (inprogressDataCheck && inprogressDataCheck.length === 0 && firstUncompleteCourse) {
        const firstChildData: any = this.widgetSvc.getFirstChildInHierarchy(firstUncompleteCourse)
      ...
      if (content.completionPercentage === 100 && inprogressDataCheck && inprogressDataCheck.length === 0 && !firstUncompleteCourse) {
        const firstChildData: any = this.widgetSvc.getFirstChildInHierarchy(content)
        const childEnrollmentData = enrolmentList.find((el: any) =>
  ...
  public mapModuleDurationAndProgress(content: NsContent.IContent | null, parent: NsContent.IContent | null) {
    if (content && content.children) {
      ...
      content.children.forEach((item: NsContent.IContent) => {
  ...
  getCalculationsFromChildren(item: NsContent.IContent) {
    item['duration'] = item.children.reduce((sum: any, child: any) => {
