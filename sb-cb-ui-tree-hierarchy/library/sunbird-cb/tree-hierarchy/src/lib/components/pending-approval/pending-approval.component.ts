...
export class PendingApprovalComponent implements OnInit {
  ...
  ngOnInit() {
      ...
      this.frameworkService.getFrameworkInfo().subscribe(res => {
        this.categories = res.result.framework.categories.map((d: any) => d.code)
      })
  }
  ...
  createApprovalList(updateFieldValues :any){
    let updateFileds:any = []
    let categories = []
    let approvalList: any = []
    updateFileds = JSON.parse(updateFieldValues);
    categories =  updateFileds.map((fd: any) => fd.category)
    categories =  this.removeDuplicates(categories)
    categories.forEach((c) => {
      let temp: any = {name:'', terms:[], children: []}
      temp.name = c,
      temp.children = updateFileds.filter((term: any) => term.category === c)
  ...
  getTerms(terms: any){
      const temp = terms.map((t:any) => t.name);
      temp.shift()
      return temp
  }
