...
jest.mock(
  "@sunbird-cb/collection/src/public-api",
  () => ({
...
describe("LearnSearchComponent", () => {
  ...
  const mockSearchV3Service = {
    ...
    searchCommunity: jest.fn().mockReturnValue(
      of({
        result: {
          search_results: {
            data: [],
    ...
    enrollment: jest.fn().mockReturnValue(
      of({
        result: {
          courses: []
        }
  ...
  it("should update no result message", () => {
    ...
    component.updateNoResultMessage = jest.fn(searchTerm => {
      mockTranslate.get("learnsearch.noResultFound", { searchTerm }).subscribe((translatedText: string) => {
