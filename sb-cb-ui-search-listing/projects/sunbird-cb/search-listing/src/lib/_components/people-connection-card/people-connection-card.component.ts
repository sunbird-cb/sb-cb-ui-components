...
export class PeopleConnectionCardComponent {
  ...
  connetToUser() {
    const req = {
      ...
      userDepartmentFrom: this.currentUser && this.currentUser.departmentName ? this.currentUser.departmentName : "",
      userIdTo: this.unmappedUser.userId,
      userNameTo: this.user.userId || this.user.identifier || this.user.wid,
      userDepartmentTo: this.unmappedUser.profileDetails?.professionalDetails.length
        ? this.unmappedUser.profileDetails?.professionalDetails[0]?.designation
        : this.unmappedUser.rootOrgName
    };
    this.networkService.createConnection(req).subscribe(
      () => {
