...
export class PaginationComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      changes["totalItemsCount"].currentValue !== changes["totalItemsCount"].previousValue
    ) {
      this.paginationInListing();
    }
  }
  ...
  navigateToNextPage(page: number) {
    if (page <= (this.pagination.paginationLength && this.pagination.paginationLength[this.pagination.paginationLength.length - 1])) {
